...
| v2.8.1 | 2026-04-04 | **视觉评测 Bug 修复 + 模型管理增强**。🔸 **Bug 修复**：`asciiPixelFont` 补全缺失的数字 `0`/`4`/`6`/`7`/`9` 像素字形（此前使用这些数字的 OCR 测试图会渲染空白）；修复 `runBenchmarkOffline` 中 `maxTotal` 计算——跳过的视觉维度（如 `scene_natural.png` 加载失败时）不再虚增总分母，改为基于实际完成维度数计算百分比；修复模型列表评测分显示缺少视觉维度图标（`renderModelBenchmark` 补全 9 个 vision 维度的 emoji 映射）；🔸 **云端模型评测列**：云端模型（`:cloud`）表格新增「评测」列，显示评测百分比 + 各维度小图标评分，与本地模型一致；🔸 **全列排序**：云端和本地模型两个表格的所有列（类型/能力/评测/参数/量化）均支持点击表头排序，评测列按百分比数值排序 | v2.8.0 | 2026-04-04 | **视觉评测维度全面扩展**（9 → 从 4 项扩展为 9 项）。🔸 **OCR 4 子维度**：英文OCR（HELLO）、中文OCR（你好世界）、中英混合OCR（AI人工智能 + 上海HELLO）、表格文字识别（中国/北京/上海 + 数值表格），全部通过 8×8 像素点阵字体代码生成，零外部依赖；🔸 **图表数据读取**：代码生成 4 组柱状图（A=8, B=5, C=3, D=12），验证模型读取图表标签和数值的能力；🔸 **自然场景理解**：加载 AI 生成的自然风景图（`testdata/scene_natural.png`），验证模型识别天空/云/草地/房屋/太阳/树等场景元素的能力；🔸 **像素字体系统**：新增 `cnPixelFont`（14 个汉字 8×8 点阵）+ `asciiPixelFont`（含数字/符号）+ `drawString` 统一渲染函数，支持中英文混排；🔸 多模态模型满分从 100 分提升至 150 分（6 文本 + 9 视觉），百分制统一对比不受影响 |
| v2.7.0 | 2026-04-03 | **多模态评测能力**。🔸 自动检测 vision 模型（通过 `/api/show` 的 `details.families` 中 `clip`/`mmproj` 标记），支持评测时追加 4 个视觉维度；🔸 新增 `GenerateChatWithImages` 方法，支持向 Ollama `/api/chat` 发送 base64 编码图片；🔸 4 个视觉评测维度：**图形识别**（红圆/蓝方/绿三角）、**文字识别**（OCR 提取 HELLO）、**颜色理解**（4 色矩形+亮度判断）、**视觉计数**（7 个圆点计数）；🔸 测试图片通过 Go `image` 标准库运行时生成（纯代码，零外部依赖）；🔸 纯文本模型评测 6 维度 60 满分，多模态模型评测 10 维度 100 满分，排行榜按百分制统一对比；🔸 前端：评测结果表格动态列头（不同模型可能有不同维度数），vision 维度用分隔线+👁标记区分；模型名旁显示「多模态」标签；评测详情弹窗区分文本/视觉维度 |
