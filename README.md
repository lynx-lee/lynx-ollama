...
| 版本 | 日期 | 变更 |
|------|------|------|
| v1.7.3 | 2026-03-16 | **Web healthcheck 轻量化**。`ollama-web` 容器的 Docker healthcheck 从 `/api/health`（每次调用 `IsAPIReady` + `GetVersion` 向 Ollama 发送 `GET /` + `GET /api/version`）改为新增的 `/api/ping` 端点（仅返回 `{"status":"ok"}`，零外部调用），彻底消除无客户端时 Web 容器 healthcheck 产生的 Ollama API 请求 |
